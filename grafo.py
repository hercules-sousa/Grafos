...
class Grafo:
    ...
    def vertices_nao_adjacentes(self):
        ...
        for x in self.N:
            for y in self.N:
                edge = x + self.SEPARADOR_ARESTA + y

                if edge not in self.A.values() and edge[::-1] not in self.A.values():
                    edges.append(edge)

        return edges

    def ha_paralelas(self):
        array = list(self.A.values())

        for x in range(len(array)):
            for y in range(x + 1, len(array)):
                if array[x] == array[y] or array[x] == array[y][::-1]:
                    return True
    ...
    def grau(self, vertex):
        ...
        for x in self.A.values():
            if x[0] == vertex or x[2] == vertex:
                count += 1

        return count

    def arestas_sobre_vertice(self, vertex):
        return [x for x in self.A.keys() if vertex in self.A[x]]

    def eh_completo(self):
        array = list()
        edges = set(self.A.values())
        size = len(self.N)
